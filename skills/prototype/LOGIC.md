...
Put the actual logic that answers the question in a single `<script>` block written as a small pure module. The whole prototype remains throwaway. Its value is the contract it reveals, not code to copy into production.
...
Pick whichever shape best fits the question being asked, *not* whichever is easiest to wire to a page. Keep it pure: no DOM, no `document`, no button handlers reaching inside it. The page calls into it; nothing flows the other direction. This makes the learned state and transition contract clear enough to implement normally after the prototype.
...
Once the prototype has answered its question, record the accepted state and transition contract. Keep or delete the HTML file according to the current request. Any production implementation is a separate normal change with its own tests and verification.
...
- **Don't ship prototype code into production.** The page and its logic were written to answer a question without production tests or safeguards. Keep the learned contract and implement it normally.
