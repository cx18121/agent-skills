#!/usr/bin/env node
// Modified by Charlie Xue on 2026-09-01 for Pi and shared Agent Skills compatibility.
/**
 ...
 */
function generatePinnedSkill(command, metadata, harness) {
  const desc = metadata[command]?.description || `Shortcut for the Impeccable ${command} workflow.`;
  const hint = metadata[command]?.argumentHint || '[target]';
  const codexFrontmatter = `metadata:\n  argument-hint: "${hint}"`;
  const agentSkillsFrontmatter = `argument-hint: "${hint}"\nuser-invocable: true\ndisable-model-invocation: true`;
  const providerFrontmatter = harness === '.codex'
    ? codexFrontmatter
    : harness === '.agents'
      ? `${codexFrontmatter}\n${agentSkillsFrontmatter}`
      : agentSkillsFrontmatter;
...
Load the \`impeccable\` skill, run its \`${command}\` branch, and pass along the arguments supplied with this shortcut.
`;
}
...
function pin(command, projectRoot) {
  ...
  for (const skillsDir of harnessDirs) {
    ...
    const content = generatePinnedSkill(command, metadata, harness);
