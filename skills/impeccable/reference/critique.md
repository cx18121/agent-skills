<!-- Modified by Charlie Xue on 2026-09-01 to add current Pi review tooling. -->
...
Pi sub-agent gate: follow the Pi runtime policy in `SKILL.md`. A and B are genuinely separate assessments, so run them as distinct generic children and wait for both before synthesis. If spawning fails, run sequentially and disclose the exact degraded reason.

Codex CLI sub-agent gate (not Pi using an OpenAI Codex model):
- Follow the Codex CLI permission model before spawning.
- If permission is declined or spawning is unavailable, run sequentially and disclose the degraded method.

Every child prompt contains cwd, target, live URL, references, product context, and output contract.
...
Pi browser note: Prefer Agent Browser for browser development when suitable. Otherwise use Pi's current browser and UI tools, and use targeted JavaScript only when the observable browser surface cannot provide the evidence. Do not use obsolete Codex Browser calls such as `visibility.set` or `tab.dev.logs`.
...
**After presenting findings**, use the active harness's structured question tool based on what was actually found. Ask directly in chat only when no structured tool exists. These answers will shape the action plan.
