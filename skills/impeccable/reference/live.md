<!-- Modified by Charlie Xue on 2026-09-01 to add Pi process and mutation ownership. -->
...
Codex CLI: use its required sandbox permission for localhost and package-manager access.
...
Harness policy:
- **Pi**: run the app server, helper server, and one-shot poll with the `process` tool. Use completion or event-pattern notifications rather than polling process output.
...
In Pi, the parent applies source edits inline with this contract. In harnesses that permit child writers, the named manual-edit applier may receive cwd, scripts path, event id, page URL, chunk/deadline, `batch`, `evidencePath`, and the canonical JSON result schema. The child must not poll or reply.
