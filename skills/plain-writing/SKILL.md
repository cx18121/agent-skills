...
name: plain-writing
description: >-
  Write or revise Charlie's prose in his plain style: simple words, complete sentences, no dashes, jargon, analogies, or filler. Use for any prose drafted for Charlie and for requests to simplify, tighten, or reword text. Do not apply it to code.
---
...
The user's AGENTS.md has a general writing rule called "Write Like a Person".
This skill is the more specific version of that rule for prose, and it wins
where the two disagree. For example, AGENTS.md allows an occasional dash or
colon, but the rules below do not, so follow the rules below.
