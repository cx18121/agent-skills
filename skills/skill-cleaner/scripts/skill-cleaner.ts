...
const model = argValue("--model", process.env.PI_MODEL || "unspecified");
...
function flagValues(name: string): string[] {
  const raw = process.argv.slice(2);
  return raw.flatMap((arg, index) => (arg === name && raw[index + 1] ? [raw[index + 1]] : []));
}

const requestedRoots = flagValues("--root");
...
function codexModelContext(modelName: string): {
  ...
  if (modelName === "unspecified") {
    return { tokens: 272_000, source: "fallback:272k (no model specified)", effectivePercent: null };
  }
  ...
  return { tokens: 272_000, source: "fallback:272k", effectivePercent: null };
}
...
function parseFrontmatter(file: string): {
  ...
  for (let i = 0; i < fm.length; i++) {
    ...
    if (key === "disable-model-invocation") disableModelInvocation = raw.trim() === "true";
    if (key === "description") {
...
function render(skills: Skill[], usage: Map<string, Usage>, logFiles: string[]): string {
  ...
  const modelVisible = enabled.filter((skill) => skill.modelInvocable);
  ...
  const longDescriptions = modelVisible
  ...
  const unused = enabled
    ...
    .slice(0, 80);
  const totalLineChars = modelVisible.reduce((sum, skill) => sum + skill.lineChars, 0);
  const totalDescChars = modelVisible.reduce((sum, skill) => sum + skill.descChars, 0);
  const budget = skillBudget(modelVisible);
  const lines: string[] = [];
  lines.push("# Skill Cleaner Report", "");
  lines.push(`generated: ${new Date().toISOString()}`);
  lines.push(`months: ${months}`);
  lines.push(`skills: ${skills.length} discovered, ${enabled.length} enabled, ${modelVisible.length} model-visible`);
  ...
  if (logFiles.length === 0) {
    lines.push("- not evaluated (no transcript logs scanned)");
  } else {
  ...
  for (const [root, list] of [...roots.entries()].sort((a, b) => b[1].length - a[1].length)) {
    ...
    const manualOnly = list.filter((skill) => !skill.modelInvocable).length;
    const qualifiers = [
      disabled ? `${disabled} disabled` : "",
      manualOnly ? `${manualOnly} manual-only` : "",
    ].filter(Boolean);
    lines.push(`- ${root}: ${list.length} skills${qualifiers.length ? `, ${qualifiers.join(", ")}` : ""}`);
  }
  return lines.join("\n");
}
...
const consideredSkills = skills.filter(
  (skill) => (skill.enabled || includeAll) && skill.modelInvocable,
);
